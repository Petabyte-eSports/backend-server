...
export class PostLikeConsumerNotification {
  ...
  async transcode(job) {
    ...
    await this.firebase.sendNotification(
      [post.user.notification_token], // Wrap the token in an array
      'Post Commented',
      'Your post has been commented on by ' + likedBy.name,
    );
    ...
    users.forEach((user) => {
      if (user.id !== likedBy.id) {
        this.firebase.sendNotification(
          [user.notification_token], // Wrap the token in an array
          'Post Commented',
          likedBy.name + ' commented on a post you commented on',
        );
      }
    });
